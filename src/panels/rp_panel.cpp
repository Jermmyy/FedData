#include "rp_panel.hpp"
#include "implot.h"
#include "imgui.h"
#include <vector>
#include <algorithm>

void renderRPPanel(const RPClient& rp) {
    const auto& data = rp.data();
    ...
    if (showChart) {
        ...
        if (ImPlot::BeginPlot("RP Volume", ImVec2(-1, -1))) {
            ImPlot::SetupAxes("Day", "Amount ($B)");
            ImPlot::SetupAxisTicks(ImAxis_X1, x.data(), (int)x.size()); // integer day ticks
            ImPlot::PlotBars("RP Amount", x.data(), y.data(), (int)data.size(), 0.5);
            ImPlot::EndPlot();
        }
    }
    else {
        // Raw data table
        ImGui::BeginTable("RP Table", 2, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg);
